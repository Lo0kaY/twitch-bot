...
token = 'ODMzMjg1NzMzMjEyODgwOTA2.YHwH0A.K5Ke2SsRP2P9LPDPqE7LCWt1I_c'
prefix = '/'
...
administrators = [804623736137449482, 508410663920468010]
chat_channel = 833089351222165554
bots_channel = 833089351222165554
queue = []
            ...
            if str(channel.lower()) in tfollow_cooldown and ctx.author.id not in administrators:
                ...
                try:
                    ...
                    if ctx.author.id in administrators:
                        ...
                        max_amount += 5000
                    premium = discord.utils.get(ctx.guild.roles, name='Kings')
                    if premium in ctx.author.roles:
                        max_amount += 4975
                    diamond = discord.utils.get(ctx.guild.roles, name='Premium')
                    if diamond in ctx.author.roles:
                        max_amount += 2475
                    gold = discord.utils.get(ctx.guild.roles, name='Diamand')
                    if gold in ctx.author.roles:
                        max_amount += 1475
                    silver = discord.utils.get(ctx.guild.roles, name='Gold')
                    if silver in ctx.author.roles:
                        max_amount += 675
                    bronze = discord.utils.get(ctx.guild.roles, name='Silver')
                    if bronze in ctx.author.roles:
                        max_amount += 175
                    booster = discord.utils.get(ctx.guild.roles, name='Bronze')
